...
const marked = new marked$1.Marked({
    walkTokens(token) {
        if (!context || token.type != "code")
            return;
        let lang = context.language && context.language(token.lang);
        if (!lang) {
            let viewLang = context.view.state.facet(language.language);
            if (viewLang && viewLang.name == token.lang)
                lang = viewLang;
        }
        if (!lang)
            return;
        let highlighter = { style: tags => language.highlightingFor(context.view.state, tags) };
        let result = "";
        highlight.highlightCode(token.text, lang.parser.parse(token.text), highlighter, (text, cls) => {
...
class LSPPlugin {
    ...
    */
    view, { client, uri, languageID }) {
        ...
        this.uri = uri;
        if (!languageID) {
            let lang = view.state.facet(language.language);
            languageID = lang ? lang.name : "";
        }
        client.workspace.openFile(uri, languageID, view);
        this.syncedDoc = view.state.doc;
        this.unsyncedChanges = state.ChangeSet.empty(view.state.doc.length);
    }
    /**
    ...
    */
    clear() {
        ...
        this.unsyncedChanges = state.ChangeSet.empty(this.view.state.doc.length);
    }
    /**
    @internal
    */
...
const lspPlugin = view.ViewPlugin.fromClass(LSPPlugin);
...
const lspTheme = view.EditorView.baseTheme({
...
class WorkspaceMapping {
    ...
    constructor(client) {
        ...
        */
        ...
        for (let file of client.workspace.files) {
            this.mappings.set(file.uri, state.ChangeSet.empty(file.doc.length));
    ...
    */
    ...
    mapPos(uri, pos, assoc = -1, mode = state.MapMode.Simple) {
        let changes = this.getMapping(uri);
        return changes ? changes.mapPos(pos, assoc, mode) : pos;
    }
    mapPosition(uri, pos, assoc = -1, mode = state.MapMode.Simple) {
...
function serverCompletion(config = {}) {
    let result;
    if (config.override) {
        result = [autocomplete.autocompletion({ override: [serverCompletionSource] })];
    }
    else {
        let data = [{ autocomplete: serverCompletionSource }];
        result = [autocomplete.autocompletion(), state.EditorState.languageData.of(() => data)];
    }
    if (config.validFor)
...
function shouldTriggerCompletion(plugin, character) {
    ...
    if (triggers !== undefined && triggers.indexOf(character) > -1)
        ...
        return {
            ...
            options: result.items.map(item => {
                ...
                if (insertTextFormat == 2 /* Snippet */) {
                    option.apply = (view, c, from, to) => autocomplete.snippet(text.replace(/\$(\d+)/g, "${$1}"))(view, c, from, to);
                }
            ...
            map: (result, changes) => (Object.assign(Object.assign({}, result), { from: changes.mapPos(result.from) })),
        };
    }, err => {
...
function hoverTooltips(config = {}) {
    return view.hoverTooltip(lspTooltipSource, {
...
function renderCode(plugin, code) {
    ...
    if (!lang) {
        let viewLang = plugin.view.state.facet(language.language);
    ...
    let result = "";
    highlight.highlightCode(value, lang.parser.parse(value), { style: tags => language.highlightingFor(plugin.view.state, tags) }, (text, cls) => {
        result += cls ? `<span class="${cls}">${escHTML(text)}</span>` : escHTML(text);
    }, () => {
        result += "<br>";
    });
...
const formatDocument = view => {
    ...
    plugin.client.withMapping(mapping => getFormatting(plugin, {
        tabSize: language.getIndentUnit(view.state),
        insertSpaces: view.state.facet(language.indentUnit).indexOf("\t") < 0,
    }).then(response => {
        if (!response)
            return;
...
method.
*/
const renameSymbol = view$1 => {
    let wordRange = view$1.state.wordAt(view$1.state.selection.main.head);
    let plugin = LSPPlugin.get(view$1);
    if (!wordRange || !plugin || plugin.client.hasCapability("renameProvider") === false)
        return false;
    const word = view$1.state.sliceDoc(wordRange.from, wordRange.to);
    let panel = view.getDialog(view$1, "cm-lsp-rename-panel");
    if (panel) {
        ...
        let { close, result } = view.showDialog(view$1, {
            label: view$1.state.phrase("New name"),
            input: { name: "name", value: word },
            focus: true,
            submitLabel: view$1.state.phrase("rename"),
            class: "cm-lsp-rename-panel",
        });
        result.then(form => {
            view$1.dispatch({ effects: close });
            if (form)
                doRename(view$1, form.elements.namedItem("name").value);
        });
    }
    return true;
};
...
const signaturePlugin = view.ViewPlugin.fromClass(class {
    constructor() {
...
const signatureState = state.StateField.define({
    create() { return null; },
    update(sig, tr) {
        ...
        if (sig && tr.docChanged)
            return new SignatureState(sig.data, sig.active, Object.assign(Object.assign({}, sig.tooltip), { pos: tr.changes.mapPos(sig.tooltip.pos) }));
        return sig;
    },
    provide: f => view.showTooltip.from(f, sig => sig && sig.tooltip)
});
const signatureEffect = state.StateEffect.define();
...
const showSignatureHelp = view => {
    ...
    if (!plugin) {
        view.dispatch({ effects: state.StateEffect.appendConfig.of([signatureState, signaturePlugin]) });
...
function signatureHelp(config = {}) {
    return [
        signatureState,
        signaturePlugin,
        config.keymap === false ? [] : state.Prec.high(view.keymap.of(signatureKeymap))
    ];
}
...
const referencePanel = state.StateField.define({
    create() { return null; },
    update(panel, tr) {
        for (let e of tr.effects)
    ...
    provide: f => view.showPanel.from(f)
});
const setReferencePanel = state.StateEffect.define();
function displayReferences(view, locs, mapping) {
    ...
    let effect = view.state.field(referencePanel, false) === undefined
        ? state.StateEffect.appendConfig.of(referencePanel.init(() => panel))
...
const autoSync = view.ViewPlugin.fromClass(class {
    constructor() {
        this.pending = -1;
    }
    update(update) {
    ...
    return {
        ...
        notificationHandlers: {
            "textDocument/publishDiagnostics": (client, params) => {
                ...
                if (!view || !plugin)
                    return false;
                view.dispatch(lint.setDiagnostics(view.state, params.diagnostics.map(item => {
                    var _a;
                    return ({
...
function languageServerSupport(client, uri, languageID) {
    return [
        ...
        serverCompletion(),
        hoverTooltips(),
        view.keymap.of([...formatKeymap, ...renameKeymap, ...jumpToDefinitionKeymap, ...findReferencesKeymap]),
        signatureHelp(),
    ];
}
/**
...
exports.WorkspaceMapping = WorkspaceMapping;
exports.closeReferencePanel = closeReferencePanel;
exports.findReferences = findReferences;
exports.findReferencesKeymap = findReferencesKeymap;
exports.formatDocument = formatDocument;
exports.formatKeymap = formatKeymap;
exports.hoverTooltips = hoverTooltips;
exports.jumpToDeclaration = jumpToDeclaration;
exports.jumpToDefinition = jumpToDefinition;
exports.jumpToDefinitionKeymap = jumpToDefinitionKeymap;
exports.jumpToImplementation = jumpToImplementation;
exports.jumpToTypeDefinition = jumpToTypeDefinition;
...
exports.prevSignature = prevSignature;
exports.renameKeymap = renameKeymap;
exports.renameSymbol = renameSymbol;
exports.serverCompletion = serverCompletion;
exports.serverCompletionSource = serverCompletionSource;
exports.serverDiagnostics = serverDiagnostics;
exports.showSignatureHelp = showSignatureHelp;
exports.signatureHelp = signatureHelp;
exports.signatureKeymap = signatureKeymap;
